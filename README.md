...
[minimax_visualizer.py](minimax_visualizer.py) is both a marimo notebook and an executable Python file. After selecting a problem, click **Next** or press **Space** to advance one step, click **Previous** or press **↑** to go back, and click **Restart** to return to the beginning. Switching problems resets the progress. The original leaf scores remain visible at all times, but only leaves actually read by the search count as visited.
